                    ...
                    <div class="card-body tab-content">
                        <div class="tab-pane active" id="profile">
                            ...
                            <form action="${pageContext.request.contextPath}/controller" method="POST">
                                ...
                                <div class="form-group">
                                    ...
                                    <input pattern="[A-ZА-Я][a-zа-я\-]{1,32}"
                                           title="<fmt:message key="text.profileSettings.placeholder.firstName"/>"
                                           class="form-control" name="firstName" id="firstName"
                                           aria-describedby="firstNameHelp"
                                           placeholder="Enter your firstName" value="${user.firstName}">
                                    <small id="firstNameHelp" class="form-text text-muted"><fmt:message
                                            key="text.profileSettings.infoFirstName"/></small>
                                </div>
                                <hr>
                                <div class="form-group">
                                    <label for="lastName"><fmt:message key="text.registration.lastName"/></label>
                                    <input pattern="[A-ZА-Я][a-zа-я\-]{1,32}"
                                           title="<fmt:message key="text.profileSettings.placeholder.lastName"/>"
                                           class="form-control" name="lastName" id="lastName"
                                           aria-describedby="lastNameHelp"
                                           placeholder="Enter your lastName" value="${user.lastName}">
                                    <small id="lastNameHelp" class="form-text text-muted"><fmt:message
                                            key="text.profileSettings.infoLastName"/></small>
                                </div>
                                <hr>
                                <div class="form-group">
                                    ...
                                    <small id="emailHelp" class="form-text text-muted"><fmt:message
                                            key="text.profileSettings.infoEmail"/></small>
                                </div>
                                <button type="button" class="btn btn-primary" data-toggle="modal"
                                        data-target="#infoModal"><fmt:message key="text.profileSettings.update"/>
                                </button>
                                <button type="reset" class="btn btn-light"><fmt:message
                                        key="text.profileSettings.reset"/></button>
                                <c:if test="${not empty errorInfo}">
                                    <div class="alert alert-danger" role="alert" style="margin: 20px"><fmt:message
                                            key="text.profileSettings.invalidData"/></div>
                                </c:if>
                                <!-- Modal -->
                                    ...
                                    <div class="modal-dialog" role="document">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title" id="infoModalLabel"><fmt:message
                                                        key="text.profileSettings.pleaseConfirm"/></h5>
                                                <button type="button" class="close" data-dismiss="modal"
                                                        aria-label="Close">
                                                    <span aria-hidden="true">&times;</span>
                                                </button>
                                            </div>
                                            <div class="modal-body" style="margin-bottom: 25px"><fmt:message
                                                    key="text.profileSettings.confirm"/></div>
                                            <div class="modal-footer">
                                                <button type="button" class="btn btn-secondary" data-dismiss="modal">
                                                    <fmt:message key="text.profileSettings.close"/></button>
                            ...
                            <form action="${pageContext.request.contextPath}/controller" method="POST">
                                ...
                                <div class="form-group">
                                    ...
                                    <input pattern="^(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z])(?=\S+$).{6,25}$"
                                           ...
                                           required>
                                    <br>
                                    <label class="d-block text-danger"><fmt:message
                                            key="text.profileSettings.deleteAccount"/></label>
                                    <p class="text-muted font-size-sm"><fmt:message
                                            key="text.profileSettings.deleteAccountInfo"/></p>
                                </div>
                                <button type="button" class="btn btn-primary" data-toggle="modal"
                                        data-target="#deleteModal"><fmt:message
                                        key="text.profileSettings.deleteAccount"/>
                                </button>
                                <!-- Modal -->
                                <div class="modal fade" id="deleteModal" tabindex="-1" role="dialog"
                                    ...
                                    <div class="modal-dialog" role="document">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title" id="deleteModalLabel"><fmt:message
                                                        key="text.profileSettings.pleaseConfirm"/></h5>
                                                <button type="button" class="close" data-dismiss="modal"
                                                        aria-label="Close">
                                                    <span aria-hidden="true">&times;</span>
                                            ...
                                            <div class="modal-body" style="margin-bottom: 25px;"><fmt:message
                                                    key="text.profileSettings.confirmDeleteAccount"/></div>
                                            <div class="modal-footer">
                                                <button type="button" class="btn btn-secondary" data-dismiss="modal">
                                                    <fmt:message key="text.profileSettings.close"/>
                                                </button>
                                                <button type="submit" class="btn btn-danger"><fmt:message
                                                        key="text.profileSettings.deleteAccount"/></button>
                                                <c:if test="${not empty error}">
                                                    <span class="text-danger"><fmt:message
                                                            key="text.client.errorPassword"/></span>
                                                </c:if>
                                            </div>
                                        </div>
                                    </div>
                        ...
                        <div class="tab-pane" id="security">
                            ...
                            <form action="${pageContext.request.contextPath}/controller" method="POST"
                                ...
                                <div class="form-group">
                                    <label class="d-block"><fmt:message
                                            key="text.profileSettings.changePassword"/></label>
                                    <input pattern="^(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z])(?=\S+$).{6,25}$" type="password"
                                           id="oldPassword" autocomplete="current-password" name="oldPassword"
                                           class="form-control"
                                           placeholder="<fmt:message key="text.profileSettings.enterOldPassword"/>"
                                           title="<fmt:message key="text.registration.passwordError"/>"
                                           required>
                                    <hr>
                                    <input pattern="^(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z])(?=\S+$).{6,25}$" type="password"
                                           id="newPassword" autocomplete="new-password" name="newPassword"
                                           class="form-control mt-1"
                                           placeholder="<fmt:message key="text.profileSettings.enterNewPassword"/>"
                                           title="<fmt:message key="text.registration.passwordError"/>" required>
                                    <hr>
                                    <input pattern="^(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z])(?=\S+$).{6,25}$" type="password"
                                           id="confirmPassword" autocomplete="new-password" name="confirmPassword"
                                           class="form-control mt-1"
                                           title="<fmt:message key="text.registration.passwordError"/>"
                                           placeholder="<fmt:message key="text.profileSettings.enterNewPassword"/>"
                                           required>
                                    <hr>
                                    <button type="submit" id="updatePasswordButton" class="btn btn-primary"
                                            data-target="#passwordModal"><fmt:message
                                            key="text.profileSettings.updatePassword"/>
                                    </button>
                                </div>
                            </form>
                            <hr>
                        ...
                        <div class="tab-pane" id="billing">
                            ...
                            <div class="form-group">
                                ...
                                <label class="d-block mb-0"><fmt:message
                                        key="text.profileSettings.wallet.payment"/></label>
                                <div class="row" style="margin: 20px">
                                    <form action="${pageContext.request.contextPath}/controller" method="POST">
                                        <input type="hidden" name="command" value="add_balance"/>
                                        <input type="hidden" name="amount" value="250">
                                        <button class="btn btn-info" type="submit" style="margin: 20px"><fmt:message
                                                key="text.profileSettings.add"/> 250💵
                                        </button>
                                    </form>
                                    <form action="${pageContext.request.contextPath}/controller" method="POST">
                                        <input type="hidden" name="command" value="add_balance"/>
                                        ...
                                        <button class="btn btn-info" type="submit" style="margin: 20px"><fmt:message
                                                key="text.profileSettings.add"/> 500💵
                                        </button>
                                    </form>
                                    <form action="${pageContext.request.contextPath}/controller" method="POST">
                                        <input type="hidden" name="command" value="add_balance"/>
                                        ...
                                        <button class="btn btn-info" type="submit" style="margin: 20px"><fmt:message
                                                key="text.profileSettings.add"/> 1000💵
                                        </button>
                                    </form>
                                    <form action="${pageContext.request.contextPath}/controller" method="POST">
                                        <input type="hidden" name="command" value="add_balance"/>
                                        ...
                                        <button class="btn btn-info" type="submit" style="margin: 20px"><fmt:message
                                                key="text.profileSettings.add"/> 2500💵
                                        </button>
                                    </form>
                                </div>
                            </div>
